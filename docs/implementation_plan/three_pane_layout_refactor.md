...
| *YYYY-MM-DD* | • **ScriptView (Task #5 Node Horizontal Centering)**: Modified `CanvasWrapper.jsx` to pass its `clientWidth` as `canvasWidth` to children and set its own viewport x-offset to `0` for `useFitView=false` cases (like `ScriptView`). Updated `TurnNode.jsx` to report its `offsetWidth` via a new `onWidthReport` callback. Enhanced `TurnCanvas.jsx` to accept `canvasWidth`, manage a `nodeWidths` state, and pass `onWidthReport` to nodes; its main layout `useEffect` now also depends on `canvasWidth` and `nodeWidths`, triggering a call to `calculateNodesAndEdges` with all actual dimensions. Finally, refactored `TurnCanvas.utils.js` (`calculateNodesAndEdges`) to use the provided `canvasWidth`, `nodeWidths` map, and `nodeHeights` map to calculate precise `x` (for horizontal centering of each node) and `y` (for dynamic vertical spacing) positions. This ensures each node is centered independently, resolving slanted edges. |
| *YYYY-MM-DD* | • **ScriptView (Task #5 RSP Styling)**: Refactored `RightSidePanel.jsx` to use the shared `buttonStyle` from `../styles/commonStyles.js` for its "Export" and "Edit" buttons, ensuring consistent button appearance with `LoadView`. Added `fontFamily: '"Inter", sans-serif'` to the helper text in the `RSPIdle` state to align its typography with common application styles. This promotes visual consistency for interactive elements and text in the Right-Side Panel across different views. |
| *YYYY-MM-DD* | • **ScriptView (Task #5 RSP Styling & Layout)**: In `RightSidePanel.jsx`, explicitly set button styles (including `borderRadius: '12px'` and other core properties directly inline) for diagnostic purposes to address squared-edge issue. Added `height: '100%'` to the `RSPIdle` main `div` to improve vertical centering of its content, aiming for consistency with `LoadView`'s RSP behavior. |
| *YYYY-MM-DD* | • **Layout (Task #5 RSP Centering)**: Modified `ThreePaneLayout.tsx` to make its `panel` slot container (`<aside>`) a flex column. This enables `RightSidePanel.jsx` (which has `height: '100%'` and flex centering for its content) to correctly fill the panel height and vertically center its content (`RSPIdle` or `RSPSelected` blocks), ensuring consistent RSP content centering across views. |
| *YYYY-MM-DD* | • **ScriptView (Task #5 Editing & Metadata)**: Implemented turn editing functionality in `ScriptView`.<br>  • **Backend (`apps/api-server`)**: Updated `auth.js` to include user `id` and `name` in JWT. Ensured `turn.js` correctly captures authorship (`:User` node, `[:AUTHORED_BY]` relationship), creates new turn versions, and archives the parent turn by setting `parent.accepted = false`. Verified `script.js` fetches necessary metadata (`authorName`, `createdAt`, `commit_message`, `version`).<br>  • **Frontend (`apps/frontend`)**: Created `EditTurnForm.jsx` for text and commit message input. Integrated this into `RightSidePanel.jsx`, which now manages display/edit states, shows `RSPMetadataDisplay` consistently, and handles save/cancel logic. Save triggers API call to `PATCH /turn/:turnId`, then refreshes data via `loadScript` from `useScriptStore.js` and exits edit mode. `useScriptStore.js` actions (`startEdit`, `stopEdit`, `loadScript`) were verified/updated.<br>  • **Debugging**: Resolved `TypeError` in `RightSidePanel.jsx` by correcting `loadScript` call. Canvas now correctly reflects the edited turn by displaying the new version in place of the archived old one.<br>  • **Current Issue**: After saving an edit, the RSP correctly displays metadata for the *newly created version*. However, the conceptual goal is for the RSP to reflect the latest state of the *turn the user initiated the edit on*. Investigation needed to align `selectedTurnId` and RSP data presentation post-edit to achieve this intuitive reflection. |
| *2025-05-23* | • **ScriptView (Task #5 Canvas Update)**: Fixed issue where edited turns were appended to the canvas instead of replacing the original turn. Modified `TurnCanvas.jsx` to filter `visibleTurns` by `t.accepted === true` in addition to `t.role !== 'root'`. This ensures only the active, accepted versions of turns are rendered, correctly reflecting edits on the canvas. |
