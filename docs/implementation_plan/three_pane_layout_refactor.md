...
| *2025-05-23* | • **ScriptView (Task #5 Versioning)**: Corrected turn versioning logic in `ScriptView` RSP. Updated backend (`apps/api-server/src/routes/script.js`) Cypher query to calculate a `lineageVersion` based on a turn's creation order among its direct siblings (children of the same parent). This ensures version numbers are specific to a turn's edit history, not its sequential position in the script. The frontend now receives and displays this correct version. |
| *2025-05-23* | • **ScriptView (Task #5 Versioning Fix)**: Further refined the Cypher query in `apps/api-server/src/routes/script.js` to correctly identify parent-child relationships for version calculation. The `OPTIONAL MATCH` for the parent was corrected to `(t)-[:CHILD_OF]->(actualParentOfT:Turn)`, and the sibling count logic `SIZE([(sibling:Turn)-[:CHILD_OF]->(actualParentOfT) WHERE sibling.ts <= t.ts | sibling])` was adjusted accordingly. This resolves an issue where all turns were incorrectly showing as version 1. |
| *2025-05-23* | • **ScriptView (Task #5 Turn Sequencing)**: Fixed critical issue where editing a turn caused it to visually swap places with the subsequent turn. Updated the `PATCH /turn/:turnId` Cypher query in `apps/api-server/src/routes/turn.js` to ensure the new turn version correctly inherits its parent, role, and original timestamp (`ts`). This maintains the correct sequence of turns on the canvas after an edit. |
| *YYYY-MM-DD* | • **ScriptView (Task #5 DAG Refactor - Backend)**: Overhauled script fetching and turn editing logic in `apps/api-server` to remove reliance on `depth` property. <br>  • `GET /script/:personaId` (`routes/script.js`): Rewritten Cypher query to determine the "gold path" by traversing `CHILD_OF` relationships for `accepted:true` turns, returning an ordered list without `depth`. <br>  • `PATCH /turn/:turnId` (`routes/turn.js`): Modified Cypher query to: <br>    - Assign a new timestamp (`Date.now()`) to new turn versions. <br>    - Remove calculation and storage of `depth` and `parent_id` properties. <br>    - Correctly link the new turn version to the parent of the original edited turn using `CHILD_OF` relationship. <br>    - If editing a root turn, update the `Persona`'s `HAS_ROOT_TURN` relationship to point to the new version. <br> This ensures robust, DAG-compliant data handling, resolving issues of incorrect node ordering, duplicates, or disappearance after edits. |
| *YYYY-MM-DD* | • **ScriptView (Task #5 DAG Refactor - Frontend)**: Verified that frontend components (`TurnCanvas.utils.js`, `TurnCanvas.jsx`, `useScriptStore.js`) already operate on an ordered list of turns and do not use the `depth` property for layout or logic. No frontend code changes were required for this part of the DAG refactor. | 
