...
79. scripts/start_ngrok_tunnels.sh
   ...
   - Helper functions `print_ok`, `print_wait`, `print_error` (lines ~35-55) use colourful icons to improve UX and readability.
   - New `has_ngrok_token` multi-path checker (lines ~60-95) detects the token in ngrok v2, v3, or NGROK_AUTHTOKEN env.
